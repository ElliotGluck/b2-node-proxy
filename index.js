...
async function b2Authorize() {
  ...
  if (!keyId || !key) {
    throw new Error(
      'B2_APPLICATION_KEY_ID and B2_APPLICATION_KEY environment variables are required',
    )
  }
...
async function listFileVersions(authData, bucketId, fileName) {
  ...
  while (true) {
    ...
    const response = await fetch(
      `${authData.apiUrl}/b2api/v4/b2_list_file_versions?${params}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to list file versions: ${response.status} ${response.statusText}`,
      )
    }
...
async function deleteFileVersion(authData, fileId, fileName) {
  const response = await fetch(
    `${authData.apiUrl}/b2api/v4/b2_delete_file_version`,
    {
      method: 'POST',
      headers: {
        Authorization: authData.authorizationToken,
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to delete file version ${fileId}: ${response.status} ${response.statusText}`,
    )
  }
...
async function downloadFile(authData, fileId) {
  const response = await fetch(
    `${authData.downloadUrl}/b2api/v4/b2_download_file_by_id?fileId=${fileId}`,
    {
      method: 'GET',
      headers: {
        Authorization: authData.authorizationToken,
...
async function deduplicateAndCombineVersions(
  authData,
  bucketId,
  fileName,
  mergeEnabled,
  ...
  if (uniqueVersions.length === 1) {
    console.log(
      `One unique version after deduplication, downloading: ${uniqueVersions[0].fileId}`,
    )
  ...
  if (!mergeEnabled) {
    console.log(
      `Multiple versions but merge disabled, returning latest: ${uniqueVersions[0].fileId}`,
    )
    const fileData = await downloadFile(authData, uniqueVersions[0].fileId)
    return fileData
  }
  ...
  console.log(
    `Downloading ${uniqueVersions.length} unique versions for merging`,
  )
  const pdfBuffers = []
...
app.get('/*', async (req, res) => {
  try {
    ...
    try {
      ...
      if (!bucketId) {
        return res
          .status(404)
          .send(
            `Bucket prefix '${bucketPrefix}' not found. Available prefixes: ${Object.keys(bucketMap).join(', ')}`,
          )
      }
    ...
    let fileData = await deduplicateAndCombineVersions(
      authData,
      bucketId,
      filePath,
      isPdf && mergeVersions,
    ...
    const browserCacheTtl = parseInt(
      process.env.BROWSER_CACHE_TTL || '14400',
      10,
    ) // 4 hours default
